...
public class Worker {
	...
	public static void tryHarvest() {
		if (gc.canHarvest(curUnit.id(), Direction.Center)) {
			gc.harvest(curUnit.id(), Direction.Center);
		}
	}
	...
	public static void buildStructure(UnitType type) {
		...
		if (!buildBlueprintLocation.containsKey(curUnit.id())) {
			MapLocation open = null;
			//if (type == UnitType.Factory) {
				open = findBlueprintLocation();
			//} else  if (type == UnitType.Rocket) {
				//open = findBlueprintLocation2();
			//}
			if (open == null) {
