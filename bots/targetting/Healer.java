...
public class Healer {
    ...
    public static boolean moveHeal() {
        ...
        for (int i = 0; i < nearbyUnits.size(); i++) {
            ...
            if (unit.team() == gc.team() && gc.isHealReady(curUnit.id()) && unit.unitType() != UnitType.Rocket && unit.unitType() != UnitType.Factory) {
                if (unit.health() < maxHp && unit.health() != unit.maxHealth()) {
                    maxHp = unit.health();
                    id = unit.id();
                }
            }
    ...
    public static void findOverchargeTarget() {
        ...
        for (int i = 0; i < nearbyUnits.size(); i++) {
            ...
            if (unit.unitType() == UnitType.Ranger && Ranger.inCombat.containsKey(unit.id()) && Ranger.inCombat.get(unit.id())) {
                //maxHp = unit.health();
                id = unit.id();
                break;
            }
    ...
    public static void healWeakestAllies() {
        ...
        for (int i = 0; i < nearbyUnits.size(); i++) {
            ...
            //if can attack this enemy unit
            if (unit.team() == gc.team() && gc.isHealReady(curUnit.id()) && gc.canHeal(curUnit.id(), unit.id())) {
                if (unit.health() < minHp && unit.health() != unit.maxHealth()) {
                    minHp = unit.health();
                    id = unit.id();
                }
            }
    ...
    public static MapLocation getTarget() {
        return Player.enemyLocation[myId];
    }
